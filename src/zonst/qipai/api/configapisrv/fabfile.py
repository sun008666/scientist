...
GOPATH = GOPATH.split(":")[-1]

# 项目名称
PROJECT_NAME = "srvapisrv"
...
# @hosts("118.89.140.236:22")
@hosts("182.254.140.244:22")
def test():
    """测试环境"""

    # 远程路径：/home/tonnn/connectsrv/connetsrv_201609190306
    remote_run_name = os.path.join("/home/tonnn", PROJECT_NAME, PROJECT_NAME)
    remote_run_backup_name = os.path.join("/home/tonnn", PROJECT_NAME, "%s.backup" % PROJECT_NAME)
    remote_backup_name = os.path.join("/home/tonnn", PROJECT_NAME, "backup", \
        "%s_%s" % (PROJECT_NAME, CURRENT_DAY_STRING))

    # 创建远程文件目录
    back_up = os.path.join("/home/tonnn/", PROJECT_NAME, "backup")
    ...
    # 重启服务
    run("sudo supervisorctl restart srvapisrv")
