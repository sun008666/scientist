...
import (
	...
	"strings"
	"time"
	"zonst/qipai/api/configapisrv/hwobs/examples"
	"zonst/qipai/api/configapisrv/hwobs/obs"
)
...
const (
	...
	ak         = "7JPKJZGEEVGTQBOZC0WE"
	sk         = "wFe8KLnldodsvGlUQx0MoyvESuHjmKMeECqHGolY"
	bucketName = "hw-zonst-test"
	objectKey  = "object-test"
	location   = "cn-east-2"
)
...
func getObsClientByInputArg(ak, sk, endpoint string) *obs.ObsClient {
	var err error
	if obsClient == nil {
		obsClient, err = obs.New(ak, sk, endpoint)
		if err != nil {
...
func listObjects(bucketName, ak, sk, endpoint string) {
	input := &obs.ListObjectsInput{}
	//input.Bucket = bucketName
	input.Bucket = bucketName
	input.MaxKeys = 10
	//	input.Prefix = "src/"
	output, err := getObsClientByInputArg(ak, sk, endpoint).ListObjects(input)
	if err == nil {
		fmt.Printf("StatusCode:%d, RequestId:%s\n", output.StatusCode, output.RequestId)
...
func PutObject(bucketName, ak, sk, endpoint, key string, fileByte []byte) error {
	...
	input.Body = bytes.NewReader(fileByte)
	//output, err := getObsClientByInputArg(ak, sk, endpoint).PutObject(input)
	_, err := getObsClientByInputArg(ak, sk, endpoint).PutObject(input)
	if err == nil {
		//fmt.Printf("StatusCode:%d, RequestId:%s\n", output.StatusCode, output.RequestId)
...
func PutFile(bucketName, ak, sk, endpoint, key, sourceFile string) error {
	...
	//output, err := getObsClientByInputArg(ak, sk, endpoint).PutFile(input)
	_, err := getObsClientByInputArg(ak, sk, endpoint).PutFile(input)
	if err == nil {
		//fmt.Printf("StatusCode:%d, RequestId:%s\n", output.StatusCode, output.RequestId)
		//fmt.Printf("ETag:%s, StorageClass:%s\n", output.ETag, output.StorageClass)
	} else {
...
//	//	deleteBucket()
//	//listObjects(bucketName, ak, sk, endpoint)
//	//  listVersions()
...
//	//  putObject()
//	//putFile(bucketName, ak, sk, endpoint, "key1", "test1")
//	//  getObjectMetadata()
//	//getObject("object-test")
//}
